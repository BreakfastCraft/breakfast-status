		...
		<div class="col-xs-12">
			<?php if (! empty( $players ) ) : ?>
				...
				<?php foreach( $players as $player ): ?>
					<a href="#" data-toggle="tooltip" data-placement="bottom" title="<?php echo $player ?>"><img src="<?php echo plugins_url(); ?>/breakfast-status/face.php?s=24&u=<?php echo $player; ?>"></a>
				<?php endforeach; ?>
	...
	public function form( $instance ) {
		...
		<select class="widefat" id="<?php echo $this->get_field_id( 'icon' ); ?>" name="<?php echo $this->get_field_name( 'icon' ); ?>" >
			<?php foreach ( $breakfast_status_icon as $value ) : ?>
				<option value="<?php echo $value; ?>" <?php selected( $instance['icon'], $value, false ); ?>><?php echo $value;?> </option>
			<?php endforeach;?>
		</select>
		</p>
